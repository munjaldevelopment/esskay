@if ($crud->hasAccess('checker_transaction_document') && $entry->document_status == 0)
	<a href="javascript:void(0)" onclick="checkerDocumentEntry(this)" data-route="{{ backpack_url('checkerTransactionDocument/'.$entry->getKey()) }}" class="btn btn-sm btn-link" data-button-type="checkerDocument"><i class="la la-copy"></i> {{ trans('backpack::crud.checkerDocument') }}</a>
@endif
