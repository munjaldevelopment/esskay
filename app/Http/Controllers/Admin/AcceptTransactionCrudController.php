 ...
 */
class AcceptTransactionCrudController extends CrudController
{
    ...
    public function setup()
    {
        ...
        CRUD::setRoute(config('backpack.base.route_prefix') . '/accept_transaction');
        CRUD::setEntityNameStrings('Accept transaction', 'Accept transactions');
        ...
        if($list_accept_transaction)
        {
            ...
            $maker_accept_transaction = backpack_user()->hasPermissionTo('maker_accept_transaction');
            if($maker_accept_transaction)
            {
            ...
            $checker_accept_transaction = backpack_user()->hasPermissionTo('checker_accept_transaction');
            if($checker_accept_transaction)
            {
                $is_admin = backpack_user()->hasRole('Super Admin');
                if($is_admin)
                {
                    //$this->crud->addClause('where', 'document_status', '=', "0");
                    $this->crud->allowAccess(['checker_accept_transaction', 'revise', 'delete']);
                }
                else
                {
                    $this->crud->denyAccess(['revise']);
                    $this->crud->allowAccess(['checker_accept_transaction']);
                }
            }
            else
            {
                $this->crud->denyAccess(['checker_accept_transaction', 'revise', 'delete']);
            }

            $this->crud->denyAccess(['create', 'update', 'revise', 'delete']);

            $this->crud->addClause('where', 'transaction_status', '=', "1");
