...
class HomeController extends Controller
{
	...
	public function userOTP()
    {
		...
		{
			$checkRecord = \DB::table('users')->where(['phone' => $customer_name1])->first();
	...
	public function showInsightTrustee(Request $request)
    {
			...
			if($organisationStructures)
			{
				foreach($organisationStructures as $row)
				{
					...
					if($organisationStructuresChild)
					{
						...
						{
							$organisationStructureSubChildData = array();
							$organisationStructuresSubChild = \DB::table('hierarchy_structures')->where('parent_id', $rowChild->id)->orderBy('lft', 'ASC')->get();
							if($organisationStructuresSubChild)
							{
								foreach($organisationStructuresSubChild as $rowSubChild)
								{
									$organisationStructureSubSubChildData = array();
									$organisationStructuresSubSubChild = \DB::table('hierarchy_structures')->where('parent_id', $rowSubChild->id)->orderBy('lft', 'ASC')->get();
									if($organisationStructuresSubSubChild)
									{
										foreach($organisationStructuresSubSubChild as $rowSubSubChild)
										{
											$organisationStructureSubSubSubChildData = array();
											$organisationStructuresSubSubSubChild = \DB::table('hierarchy_structures')->where('parent_id', $rowSubSubChild->id)->orderBy('lft', 'ASC')->get();
											if($organisationStructuresSubSubSubChild)
											{
												foreach($organisationStructuresSubSubSubChild as $rowSubSubSubChild)
												{
		...
		$current_year = date('Y');
		return view('insight-listing-trustee', ['insightCatData' => $insightCatData, 'insightData' => $insightData, 'insightFirst' => $insightFirst, 'geographicalConData' => $geographicalConData, 'geographicalConTotalData' => $geographicalConTotalData, 'productConData' => $productConData, 'productConTotalData' => $productConTotalData, 'chart1' => $chart1, 'chart2' => $chart2, 'chart3' => $chart3, 'chart31' => $chart31, 'chart32' => $chart32, 'chart41' => $chart41, 'chart42' =>  $chart42, 'chart511' => $chart511, 'chart512' => $chart512, 'chart51' => $chart51, 'chart52' => $chart52, 'chart6' => $chart6, 'chart7' => $chart7, 'chart8' => $chart8, 'chart9' => $chart9, 'chart10' => $chart10, 'chart4001' => $chart4001, 'chart4002' => $chart4002, 'netWorthData' => $netWorthData, 'netWorthData1' => $netWorthData1, 'liquidityData' => $liquidityData, 'liquidityDataTotal' => $liquidityDataTotal,
    ...
    public function showTrusteeTransactionInfo(Request $request)
    {
		...
		else
		{
	    	...
	    	if($transactionData)
	    	{
				...
				$relevantPartyInvestorData = $relevantPartyTrusteeData = $relevantPartyLegalData = $relevantPartyArrangerData = $relevantPartyRatingData = array();

				$relevantPartyInvestorDataExists = \DB::table('transaction_relevant_parties')->leftJoin('transaction_relevant_party_trustee', 'transaction_relevant_parties.id', '=', 'transaction_relevant_party_trustee.transaction_relevant_party_id')->where('transaction_relevant_party_trustee.trustee_id',$trustee_id)->where('party_type', 'Investor')->where('transaction_relevant_parties.transaction_id',$transaction_id)->count();

				if($relevantPartyInvestorDataExists > 0)
				{
					$relevantPartyInvestorData = \DB::table('transaction_relevant_parties')->leftJoin('transaction_relevant_party_trustee', 'transaction_relevant_parties.id', '=', 'transaction_relevant_party_trustee.transaction_relevant_party_id')->where('transaction_relevant_party_trustee.trustee_id',$trustee_id)->where('party_type', 'Investor')->where('transaction_relevant_parties.transaction_id',$transaction_id)->get();
				}

				$relevantPartyTrusteeDataExists = \DB::table('transaction_relevant_parties')->leftJoin('transaction_relevant_party_trustee', 'transaction_relevant_parties.id', '=', 'transaction_relevant_party_trustee.transaction_relevant_party_id')->where('transaction_relevant_party_trustee.trustee_id',$trustee_id)->where('party_type', 'Trustee')->where('transaction_relevant_parties.transaction_id',$transaction_id)->count();

				if($relevantPartyTrusteeDataExists > 0)
				{
					$relevantPartyTrusteeData = \DB::table('transaction_relevant_parties')->leftJoin('transaction_relevant_party_trustee', 'transaction_relevant_parties.id', '=', 'transaction_relevant_party_trustee.transaction_relevant_party_id')->where('transaction_relevant_party_trustee.trustee_id',$trustee_id)->where('party_type', 'Trustee')->where('transaction_relevant_parties.transaction_id',$transaction_id)->get();
				}

				$relevantPartyLegalDataExists = \DB::table('transaction_relevant_parties')->leftJoin('transaction_relevant_party_trustee', 'transaction_relevant_parties.id', '=', 'transaction_relevant_party_trustee.transaction_relevant_party_id')->where('transaction_relevant_party_trustee.trustee_id',$trustee_id)->where('party_type', 'Legal')->where('transaction_relevant_parties.transaction_id',$transaction_id)->count();

				if($relevantPartyLegalDataExists > 0)
				{
				...
				$relevantPartyArrangerDataExists = \DB::table('transaction_relevant_parties')->leftJoin('transaction_relevant_party_trustee', 'transaction_relevant_parties.id', '=', 'transaction_relevant_party_trustee.transaction_relevant_party_id')->where('transaction_relevant_party_trustee.trustee_id',$trustee_id)->where('party_type', 'Arranger')->where('transaction_relevant_parties.transaction_id',$transaction_id)->count();

				if($relevantPartyArrangerDataExists > 0)
				{
	...
	public function showSanctionLetterInfo(Request $request)
    {
		...
		if($category_id == "new")
		{
			$sanctionLetterData = \DB::table('sanction_letters')->leftJoin('users', 'sanction_letters.user_id', '=', 'users.id')->selectRaw('sanction_letters.*, users.name as created_name')->where($where)->orderBy('id', 'DESC')->get();
		}
		else
		{
    	...
    	$current_year = date('Y');
		return view('sanction-letter-info', ['insightCatData' => $insightCatData, 'trustee_id' => $trustee_id, 'insightData' => $insightData, 'sanctionLetterData' => $sanctionLetterData, 'category_id' => $category_id]);
	}
	...
	public function approveSanctionLetter1(Request $request)
    {
		...
		$sanctionData = \DB::table('sanction_letters')->find($sanction_id);
	        ...
	        if($sanctionData->email_address)
	        {
	        	...
	        	foreach($emailAddressData as $emails)
	        	{
			        ...
			        Mail::send('emails.send_sanction_letter', $contactData, function ($message) use ($tempUserData) {
			            $message->to($tempUserData['email'])->subject("Sanction Letter approved");
			            $message->cc('munjaldevelopment@gmail.com');
			            $message->cc('milan.khadiya@skfin.in');
			            $message->from('communication@skfin.in', 'Ess Kay Fincorp');
			        });	
